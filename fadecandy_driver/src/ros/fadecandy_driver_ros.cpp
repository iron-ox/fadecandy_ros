...
FadecandyDriverRos::FadecandyDriverRos() {
  ...
  timer_ = nh.createTimer(ros::Duration(1.), &FadecandyDriverRos::timerCallback,
                          this);
  ...
  if (!initialized) {
    ...
    if (initialized) {
      ...
      libusb_get_string_descriptor_ascii(
          dev_handle, fadecandy_device_descriptor.iSerialNumber, serial_number,
          64);
...
void FadecandyDriverRos::diagnosticsCallback(
  ...
  if (FadecandyDriver::fadecandy_device != NULL) {
    diagnostic_status.summary(diagnostic_msgs::DiagnosticStatus::OK,
                              "Connected");
  } else {
